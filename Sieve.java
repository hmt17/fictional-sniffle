...
public class Sieve {
    ...
    public static void printsieve(int[] results, boolean[] prime) {
		...
		charTemp = prime[j] ? 'T' : 'F';
		System.out.print(results[j] + "(" + charTemp + ") ");
		j++;
	    }
	    System.out.println();
    }
    ...
    /**
     * @param intTemp the size of the array to return.
     */
    
    public static boolean[] getTrueArray(int intTemp) {
		boolean[] toReturn = new boolean[intTemp];
    ...
    public static int calculateMax(String[] args) {
		...
		if (args.length > 0) {
			...
			//refactored there wasn't a "throw"
			throw new IllegalArgumentException();
		}
		return toReturn;
    }
